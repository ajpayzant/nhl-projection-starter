...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Run the revised Phase 2 pipeline end to end')
...
def main() -> None:
    ...
    print('\n[1/7] NHL API official ingest')
    NHLAPIIngestor(season=args.season, season_type=args.season_type, max_games=args.max_games).run()

    print('\n[2/7] Hockey-Reference enrichment ingest')
    HockeyReferenceIngestor(season=args.season, season_type=args.season_type).run()

    print('\n[3/7] MoneyPuck advanced ingest')
    MoneyPuckIngestor(season=args.season, season_type=args.season_type, include_shot_data=args.include_shot_data).run()

    if args.date:
        print('\n[4/7] Daily Faceoff operational ingest')
        DailyFaceoffIngestor(date_str=args.date).run()
    else:
        print('\n[4/7] Daily Faceoff operational ingest skipped (no --date provided)')

    print('\n[5/7] Reference audit build')
    build_and_save_reference_outputs(args.season, season_type_label)

    print('\n[6/7] Silver cleanup build')
    build_silver_tables(args.season, args.season_type)

    print('\n[7/7] Validation summary')
    from scripts.run_phase2g_validate_outputs import main as validate_main
    import sys
    sys.argv = ['run_phase2g_validate_outputs.py', '--season', str(args.season), '--season-type', str(args.season_type)]
    validate_main()
